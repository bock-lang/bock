...
`todo()` and `unreachable()` are **`Never`-typed** prelude utility functions: as divergent expressions they may stand in for a value of any type, so `fn random_number(min: Int, max: Int) -> Int { todo() }` type-checks (`Never` is assignable to every type). Both **abort at runtime** via the Panic ambient effect (§10.5), lowering to the target's native abort (`panic!` on Rust, `panic` on Go, `throw` on JS/TS, `raise` on Python). They carry different intent and diagnostic: `todo()` marks code **not yet written** (expected to be reached during development) and aborts with a "not yet implemented" message; `unreachable()` asserts a branch is **logically impossible** and aborts if reached. Both accept an optional message argument (e.g. `todo("implement scoring")`) that is included in the abort diagnostic. (`unreachable` is additionally a reserved keyword with a dedicated grammar production per §3.4/§21.10; `todo` is a prelude function name only. The asymmetry is intentional: `unreachable()` may feed the exhaustiveness checker as a provably-dead-arm assertion, whereas `todo()` is simply a divergent stub.)
...
`core.convert` (v1) — `Into`, `From`, `TryFrom`, `Displayable`. The v1 conversion floor (DQ11): canonical `From` conversions are `Int→Float`, signed-integer widening, `Float32→Float`, and `Char→String`; `TryFrom[String]` is provided for `Int` and `Float` (narrowing and lossy numeric conversions are excluded from v1). `TryFrom` yields a fixed `ConvertError` (no generic error parameter in v1), and `TryInto` is **not** in v1 (there is no `TryFrom ⇒ TryInto` blanket). These canonical primitive→primitive conversions are **unsealed** in v1 — the §18.5 seal covers `(core trait, primitive)` *conformances*, not `From[primitive] for primitive` instances — so the matrix is additive in v1.x.
...
**Structural `Equatable` conformance (DQ29).** Records (§6.2) and enums (§6.3) conform to `Equatable` **structurally**: a record is `Equatable` iff every field type is `Equatable`, and an enum is `Equatable` iff every payload type of every variant is — both computed recursively (a recursive type assumes its own conformance while its leaves are checked). Equality is field-wise for records and tag-then-payload for enums (unit variants compare by tag alone; values of different variants are never equal). The structural conformance is a **compiler-provided default**, suppressed by an explicit `impl Equatable` for the type: the impl's `eq` then defines the type's equality everywhere `==`/`!=` apply (this is conformance synthesis with skip-if-occupied precedence — the same precedence the `From` ⇒ `Into` blanket uses — not a second method definition, so it does not conflict with the §18.3 inherent/trait method-resolution order). The compound built-ins compose conditionally: `List[T]`, `Set[T]`, and `Optional[T]` are `Equatable` iff `T` is; `Map[K, V]` iff `K` and `V` are; `Result[T, E]` iff `T` and `E` are; a tuple iff all its component types are. `Map` and `Set` equality is **order-independent** (content equality). Generic user types instantiate conditionally: `Pair[A, B]` is `Equatable` iff `A` and `B` are, decided at the use site on the instantiated type. With structural conformance in place, `==`/`!=` **gate** behind `Equatable` exactly as the ordering operators gate behind `Comparable`, and an `Equatable` *bound* (`fn dedupe[T: Equatable](items: List[T])`) is likewise satisfied by structural conformance. Four boundaries are normative:

- **Non-`Equatable` leaves poison.** A record holding a non-`Equatable` field — e.g. an `Fn` type, which has no equality — is not `Equatable`, and `==` on it is a compile-time error. The diagnostic must name the offending **field path and type** (e.g. ``field `handler` of type `Fn() -> Void` is not Equatable``) and suggest the fix (`impl Equatable for <T>` or removing the comparison).
- **Classes are excluded.** A `class` sits on the data/identity line and never conforms structurally; it gets `==`/`!=` only via an explicit `impl Equatable`.
- **`Float` fields carry the IEEE caveat.** Structural equality inherits the DQ10 matrix above with no special case: a record holding `NaN` in a `Float` field is `!=` an identically-constructed record (and `!=` itself).
- **The structural default does NOT extend to `Comparable` or `Hashable`.** Ordering has no canonical structural meaning (field order is declaration order, not semantic priority), so `Comparable` remains explicit-impl-only per the general rule above. `Hashable` synthesis is deferred to the v1.x derive-era design pass (paired with `@derive`), where the hash/eq consistency law can be designed as one unit.

**Container equality defers to element conformance (DQ31).** When a compound built-in (`List`/`Map`/`Set`/`Optional`/`Result`/tuple) is compared with `==`, each element/component comparison uses *that component type's own* `Equatable` conformance — the structural default for a structural-default element, and the element's own `eq` for an element carrying an explicit `impl Equatable`. A type has exactly one equality, and it is the same inside a container as outside it: comparing two `T` values inside a `List[T]` yields precisely what `t1 == t2` yields. For `Map`/`Set`, the element's `eq` likewise governs key-matching and membership/deduplication, not only the final comparison — a `Set` of a custom-`eq` key type runs its own identity semantics on that `eq`. This observable result is **normative and target-independent**; how a backend realizes it is **not** — a backend may compare structural-default elements with its native deep-equality (`==`, `reflect.DeepEqual`, native structural compare), since that is observably identical to element-wise structural comparison, and need fall back to an element-`eq` comparison loop only when an element type carries a custom `impl Equatable`. Provenance composes recursively: a nested container takes the native path iff *every* type in its element tree uses the structural default, and the element-`eq` path if any node carries a custom impl. A non-`Equatable` element still makes the whole container non-`Equatable` and `==` on it a compile error, on either path (the poison boundary above).
...
`bock run` — Build and execute. Default (and v1's only execution path) uses the interpreter; program arguments pass after `--`. `--watch` (hot reload) is accepted but not yet implemented in v1; `--target` (execute through a specific target language) ships alongside cross-target testing (§20.4).
`bock check` — Type check, lint, context validation. `--only=<aspect>` restricts the check to specific aspects (e.g., `--only=types`, `--only=context`, `--only=types,context`); see §20.1.1 for the aspect surface. `--brief` produces compact error output (omits source-context snippets). `--strict` forces production strictness for the check: completeness gaps that are warnings at the default development strictness (e.g. a public declaration missing `@context`; completeness is per-item, §20.1.1) become errors and a non-zero exit. Mirrors `bock build --strict`. `--format <human|json>` selects the output format: `json` emits exactly one machine-readable JSON document on stdout (the shared machine-output envelope, a versioned public contract — `format_version` 1). `bock check` exits non-zero if and only if the check produces at least one error; warnings never fail the check, and the exit code is unchanged by `--format`.
`bock test` — Run tests. Default uses interpreter (fast). `--filter <pattern>` selects tests by name. `--format <human|json>` mirrors `bock check`: `json` emits one machine-readable document (per-test results plus structured compile-error diagnostics). Cross-target testing (`--target`, `--all-targets`, `--smart`), coverage, and snapshot testing are planned for v1.x; see §20.4.
...
`bock inspect` — Read-only browsing of AI decisions, rule cache, and AI response cache. Defaults to build decisions; `--runtime` for runtime decisions, `--all` for both (with prefixed ids); `--unpinned`, `--module`, and `--type` filter the listing. `--format <human|json>` emits the listing in the shared machine-output envelope; the legacy `--json` flag emits the bare decision array (the two are mutually exclusive). Subcommands: `decisions` (default), `decision <id>`, `cache`, `rules`, `air <file>` (machine-readable lowered-AIR dump for editor tooling; its established `--json` tree shape stands apart from the shared envelope). The `--diff` view (changes since last build) is **Reserved for v1.x**.
...
`bock lsp` — Start the language server over stdio (§20.3). `--stdio` is accepted for LSP-client convention (stdio is the default and only v1 transport).
`bock mcp` — Start the MCP (Model Context Protocol) server over stdio, exposing the compiler surface (check / run / test / build / single-file cross-target conformance / inspect / diagnostic-code explanations) as tools for agentic clients (non-normative tooling note; see §20.3's sequencing note). Tool results reuse the `--format json` machine-output documents. `--stdio` is accepted for MCP-client-config convention.
...
**v1.x sequencing note (non-normative, 2026-06-09 design audit; updated 2026-07-19):** the v1.x editor-adjacent work is ordered agent-first. The lead tooling item — an MCP server exposing the compiler surface (`check` / `build` / `test` / `inspect` / conformance) as MCP tools, making any agentic environment a competent Bock environment — has **shipped** as the `bock mcp` subcommand (stdio transport, inside the CLI binary rather than a separate `bock-mcp` crate; see §20.1 "Servers" and changelog `20260719-mcp-server.md`); the human-facing panels above follow behind it. Target Preview has meanwhile shipped early as a VS Code extension feature (built on `--source-only`), as have several editor capabilities beyond the v1 floor (find-references, rename, document symbols, inlay hints, semantic tokens) — see changelog `20260610-design-audit-spec-touches.md`.
