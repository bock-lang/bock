...
## Build status (as of the release-v1.0.0 branch @ f3fe228+prep, 2026-07-02 — **v1.0.0 RELEASE-PREP**: the 13-PR dependabot backlog is DRAINED (#393–#408; majors astro 7 · @astrojs/cloudflare 14 · rustyline 18 · actions/checkout 7 · actions/cache 6 — per-PR CI green post-rebase, combined-tree main CI green at f3fe228; rustyline 18 additionally REPL-smoke-diffed against a pre-bump baseline, byte-identical) and the tree is STAMPED v1.0.0 (workspace + 17 internal-dep refs; extension 1.0.0 + CHANGELOG promoted; spec header 1.0.0/July 2026; root CHANGELOG `## v1.0.0 — 2026-07-02` promoted with `gen-changelog.sh --check` green [the release.yml gate]; REPL banner now derives from CARGO_PKG_VERSION). Publish preflight: 16/16 existing `bock-*` crates doesdev-owned on crates.io, `bock` unclaimed (first-time publish), test-harness publish=false. **Pre-tag gates remaining:** marketing copy lock (Q-getstarted-copy-lock / Q-wedge-usecases-page) + operator secrets confirmation (CRATES_IO_TOKEN / VSCE_PAT, publisher `bock-lang`) → then tag v1.0.0 → release.yml. MS-v1.0-hardening itself completed 2026-06-15/16 (Waves 1–3 + B + C + the #388–#391 tail): fmt/clippy/test/doc/mdbook clean, conformance 0 failed ×2, per-PR CI 15/15 + combined octopus gates green — **the v1.0 cross-target correctness floor is clean**; the conformance EXECUTION lane walks the whole tree [#372]; the assets-drift CI guard [#381] protects extension vocab/spec assets; `bock test` discovery skips unreadable subdirs [#383]; the textkit SDK-vendor demo [#382, `examples/sdk-vendor-demo/`] proves the equivalence guarantee ×5 two ways. Remaining non-release items: DQ32/DQ33 (Design, non-blocking) + Q-repl-let-persistence (LOW) + Q-conformance-rust-stale-binary-reuse (MED, local-only harness race — local rust-lane conformance can show phantom failures on fast boxes; CI authoritative) (both v1.x))
