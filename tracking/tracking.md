...
**H1 owned-files (confirmed):**
`compiler/crates/bock-cli/src/main.rs`,
`compiler/crates/bock-cli/src/check.rs`, `compiler/tests/cli/`.
The handoff's declaration is correct — check.rs exists. Exit-code
logic likely lives in check.rs; flag definitions in main.rs. Both
H1 and C1 touch main.rs, so conflict-avoidance still applies.
