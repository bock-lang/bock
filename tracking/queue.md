...
### Repo-governance gate (filed 2026-09-02, from the R8 dogfooding boundary review)

The boundary for a local model writing to the real repo is **version control + permissions**, not filesystem
isolation — `--dangerously-skip-permissions` is a benchmark-only artifact and dogfooding runs with permissions ON.
Both items below make that boundary mechanical rather than honor-system, and both are prerequisites for letting a
non-Claude author (Q-model-bench's fleet) open PRs against this repo.

- **[Q-required-status-checks] make the main-merge gate real: add a `required_status_checks` rule to `protect-main`** — chore/governance · **ready (OPERATOR-ONLY — must not be executed by any session)** · (external: repo ruleset, no files) · — · links Q-model-bench, Q-pr-scope-check, R8 dogfooding, CLAUDE.md "Pre-PR Verification" / "Main integration" · note: **verified against the live ruleset** — `protect-main` is `active` and contains exactly three rules: `deletion`, `non_fast_forward`, and `pull_request` with `required_approving_review_count = 0`. There is **no `required_status_checks` rule at all**. So `main` cannot be deleted or force-pushed and everything goes via PR, but **a PR merges with zero approvals and zero passing checks** — the pre-PR verification gate is something a session runs on itself, i.e. honor-system. That is fine while a trusted Orchestrator merges; it is worth **nothing** as a control against the PR's *author*, which is exactly the threat model as soon as a local model authors PRs. Proposed fix: add a `required_status_checks` rule naming the four gate jobs that already run on every PR — `rustfmt`, `clippy`, `test (ubuntu-latest / stable)`, `cargo doc`. Costs nothing (they already run) and closes the hole. Verified safe to require: those four jobs are gated on `needs.detect.outputs.has_crates == 'true'`, which probes for repo **content** (`compiler/crates/*/Cargo.toml`), not changed paths — so they run on every PR including docs/tracking-only ones and cannot wedge a merge as permanently-pending skipped checks. **`gh ruleset` / modifying branch protection is on CLAUDE.md's PROHIBITED list — no session may execute this, and no session should be dispatched to.** Recorded as an operator-only action:
  `PROPOSED: gh api --method POST repos/bock-lang/bock/rulesets/<protect-main-id>/... — add a required_status_checks rule to the protect-main ruleset requiring the contexts "rustfmt", "clippy", "test (ubuntu-latest / stable)", "cargo doc" (strict_required_status_checks_policy: false). Rationale: main currently merges on zero approvals and zero checks; this makes the existing pre-PR gate enforceable against untrusted PR authors. HUMAN RUNS THIS — prohibited for sessions.`
  The equivalent hand-edit in the GitHub UI (Settings → Rules → `protect-main` → add "Require status checks to pass") is the same change and equally operator-only. MED — cheap, and it blocks nothing today, but it gates any dogfooding round where a local model is the author.
- **[Q-pr-scope-check] mechanical scope enforcement on PRs (`Owned-Files:` block → CI check)** — feature/CI · **in-flight (parallel session; PR not yet open at filing time)** · `tools/scripts/check-pr-scope.py` + `.github/workflows/scope-check.yml` · — · links Q-model-bench (`scope_violations` axis in `tools/model-bench/harness/score.py`), Q-required-status-checks, #272 (the `examples-exec` informational-first precedent) · note: sessions declare "owned files" in their prompt and **nothing enforces it** — the declaration is prose in a transcript, invisible to CI. This is the mechanical form of the `scope_violations` axis the model-bench scorer already measures offline: same question, asked of real PRs instead of benchmark runs. Shape: parse an `Owned-Files:` block from the PR body, diff it against the PR's changed paths, report out-of-scope files. Lands **informational-first** (annotate, do not fail — the `examples-exec` precedent from #272) with a later ratchet to blocking once the false-positive rate is known; the ratchet is what makes it meaningful alongside Q-required-status-checks, since an informational check that is never required is advisory only. Link the implementing PR here when it opens.
