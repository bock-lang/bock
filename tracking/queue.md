...
_**Last reconciled 2026-06-15 — main 8f38ba6, 0 open PRs, clean, CI green. ★★★★★ WAVE C + SDK DEMO COMPLETE (#381–#386) — the v1.0-hardening correctness floor is clean.**
...
>
> **REMAINING v1.0-hardening tail (small):**
> - **Codegen FOUNDs (this pass):** `Q-go-generic-enum-codegen` (go enum marker-method receiver carries the `any` constraint → invalid go) · `Q-transpiled-test-enum-variant-import` (js/ts/py **transpiled-test** files drop enum-variant constructor symbols from imports → ReferenceError/NameError; non-test emission correct; rust/go immune)
> - **Chore:** `Q-error-code-renumbering` (E1001/E1005/E1006/W8020/E2030 double meanings — diagnostics §20.1, careful plan).
> - **→ Design:** **DQ32** (Hashable on collection keys) · **DQ33** (transitively-forwarded unbounded generics) — filed/escalated, non-blocking.
> - `D2-polish` → v1.2 (pure docs). **The v1.0 correctness floor is clean** — what's left is a couple of narrow per-backend FOUNDs + the diagnostics renumbering + 2 Design questions.
