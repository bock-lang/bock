...
## Build status (as of main e096253, 2026-06-05)
...
| `bock build`/run examples ×5 (project mode) | **CLIMBING — examples 84→92/100 (2026-06-05).** Honest exec audit on main e096253 (`tools/examples-exec-baseline.txt`): **runtime-working js 19·ts 18·py 18·rust 19·go 18 / 20 = 92/100.** The DQ27/DQ28 + class-codegen/compose/chat-protocol waves landed (#249–#260); **react-components now runs on all 5** (the last all-red example). **HONESTY:** #259 fixed a py statement-`match`→early-`return` bug that had silently truncated examples (exit 0 = false 'pass'); type-zoo py honestly flipped pass→run-FAIL (de-masking, not a regression). Remaining 8 reds: **5× guessing-game** (its own `todo()` stubs — not codegen) + calculator-ts, type-zoo-py, systems-allocator-go (all filed). Exec-gated in CI **informationally** (#204; baseline re-recorded this PR). |
