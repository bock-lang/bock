...
## Build status (as of main 09427b8, 2026-06-07)

| What | State |
|------|-------|
| `cargo test --workspace` | passing (~2730 tests, 0 failed — wave-1 backlog fan-out #271–#274) |
| `cargo clippy --workspace --all-targets -D warnings` | clean |
| `cargo fmt --all -- --check` | clean |
| `cargo doc --workspace --no-deps -D warnings` | clean (now in the pre-PR gate + CI) |
| `mdbook build docs` | clean |
| CI on `main` | green; cache via Swatinem/rust-cache@v2.9.1 (#116, faster). **Blocking gates now incl. `stdlib-fmt`** (`bock fmt --check` on stdlib core, #272 — `collections.bock`/`iter.bock` excluded pending Q-bockfmt-cfarm-comma/Q-bockfmt-utf8-panic) **and `examples-exec` (RATCHETED informational→BLOCKING, #272**, STRICT `BOCK_EXAMPLES_REQUIRE=all` + real `tsc --noEmit` on the ts row) |
