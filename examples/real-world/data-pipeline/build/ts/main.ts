export class DataPoint {
  ...
  value: number;
  category: string;
  constructor({ name, value, category }: { name: string; value: number; category: string }) {
    this.name = name;
    this.value = value;
...
export class Summary {
  ...
  total: number;
  average: number;
  constructor({ count, total, average }: { count: number; total: number; average: number }) {
    this.count = count;
    this.total = total;
...
export function normalize(data: Array<DataPoint>): Array<DataPoint> {
  let maxVal = 0.0;
...
export function scale(factor: number, data: Array<DataPoint>): Array<DataPoint> {
  return data.map(data, (dp) => new DataPoint({ name: dp.name, value: (dp.value * factor), category: dp.category }));
}

export function filterCategory(cat: string): (arg0: Array<DataPoint>) => Array<DataPoint> {
  return (data) => data.filter(data, (dp) => (dp.category === cat));
}

export function removeInvalid(data: Array<DataPoint>): Array<DataPoint> {
  return data.filter(data, (dp) => (dp.value > 0.0));
}

export function computeSummary(data: Array<DataPoint>): Summary {
  let total = 0.0;
  let count = 0;
  let fcount = 0.0;
  for (const dp of data) {
...
export function formatData(data: Array<DataPoint>): string {
  let result = "";
  let first = true;
  for (const dp of data) {
    if (first) {
...
export function buildReportPipeline(): (arg0: Array<DataPoint>) => string {
  return (composeX) => formatSummary((composeX) => computeSummary(normalize(composeX))(composeX));
}

export function applyPipeline(title: string, data: Array<DataPoint>, pipeline: (arg0: Array<DataPoint>) => string): Report {
...
function main() {
  const data: Array<DataPoint> = [new DataPoint({ name: "alpha", value: 10.0, category: "sensor" }), new DataPoint({ name: "beta", value: 25.0, category: "sensor" }), new DataPoint({ name: "gamma", value: 5.0, category: "manual" }), new DataPoint({ name: "delta", value: 40.0, category: "sensor" }), new DataPoint({ name: "epsilon", value: 15.0, category: "manual" }), new DataPoint({ name: "zeta", value: 0.0, category: "sensor" }), new DataPoint({ name: "eta", value: 30.0, category: "manual" })];
