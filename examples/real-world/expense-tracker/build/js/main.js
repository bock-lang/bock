...
const Category_Transport = Object.freeze({ _tag: "Transport" });
const Category_Housing = Object.freeze({ _tag: "Housing" });
const Category_Entertainment = Object.freeze({ _tag: "Entertainment" });
const Category_Utilities = Object.freeze({ _tag: "Utilities" });
...
class Report {
  constructor({ total, by_category, count }) {
    this.total = total;
...
export function isCategory(expense, cat) {
  return (() => {
...
export function findByCategory(expenses, cat) {
...
export function categoryTotal(expenses, cat) {
  let total = 0.0;
  for (const e of expenses) {
    if (isCategory(e, cat)) {
      total = (total + e.amount);
...
export function formatReport(report) {
  ...
  for (const key of catKeys) {
    const val = ((__m, __k) => __m.has(__k) ? { _tag: "Some", _0: __m.get(__k) } : { _tag: "None" })(report.by_category, key);
    switch (val._tag) {
      case "Some": {
        const amount = val._0;
        lines = (lines + `
...
export { Category_Entertainment, Category_Food, Category_Housing, Category_Other, Category_Transport, Category_Utilities, Expense, Report };
main();
